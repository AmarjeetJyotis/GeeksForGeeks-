...
class Solution {
  public:
    ...
    Node*solve(int * pre, int&i, int size, int mini,int maxi){
        if(i>=size){
            return NULL;
        }
        ...
        root->left=solve(pre,i,size,mini,root->data);
        root->right=solve(pre,i,size,root->data,maxi);
        return root;
    }
    Node* Bst(int pre[], int size) {
        // code here
        int index=0;
        return solve(pre,index,size,INT_MIN,INT_MAX);
    }
};
